...
candidates as (
    {% for keys in candidate_join_keys %}
        ...
        inner join match_targets
            on
                {% for key in keys %}purchased_records.{{ key }} = match_targets.{{ key }}{{ ' and ' if not loop.last }}{% endfor %}

                and (
                    match_targets.target_kind = 'crm'
                    or purchased_records.record_key > match_targets.target_id
                )
...
pairs as (
    select
        ...
        (
            ...
            and purchased_records.state_norm = match_targets.state_norm
        ) as city_match,
        (
            purchased_records.street_num is not null
            and purchased_records.street_num = match_targets.street_num
        ) as street_match,
        (purchased_records.phone10 = match_targets.phone10) as phone_match,
        (purchased_records.web_domain = match_targets.web_domain) as domain_match
    from candidates
    inner join purchased_records
        on candidates.record_key = purchased_records.record_key
    inner join match_targets
        on
            candidates.target_id = match_targets.target_id
            and candidates.target_kind = match_targets.target_kind
)
...
select
    *,
    case
        ...
        when
            (coalesce(phone_match, false) or coalesce(domain_match, false))
            and name_sim >= 0.80
            and coalesce(street_match, false)
            then 'duplicate'
    end as match_class
